class Tarif:
    def __init__(self, name: str, quantity_of_bots: int, price: int, bot_tarifs: list):# {BotTarif: int}}
        self.name = name
        self.quantity = quantity_of_bots
        self.price = price
        self.bot_tarifs = bot_tarifs
    def to_dict(self):
        dic = {'name': self.name, 'quantity': self.quantity, 'price': self.price}
        dic['bot_tarifs'] = []
        for key, value in self.bot_tarifs:
            dic['bot_tarifs'].append([key.__dict__, value])
        return dic
        # остановился здесь
        # нужно как то сериализировать словарь BotTarif
